...
class WebSocketStateManager:
    ...
    async def update_user_processing(self, user_id: str, enabled: bool) -> bool:
        ...
        try:
            ...
            logger.debug("Updated user %s processing state to %s", user_id, enabled)
            return True
    ...
    def add_user(self, user_id: str, enabled: bool = True):
        ...
        self.user_processing_enabled[user_id] = enabled
        logger.info("Added user %s to processing state (enabled: %s)", user_id, enabled)
    ...
    def clear_all_users(self):
        ...
        logger.info("Cleared all user processing states (%d users)", cleared_count)
    ...
    def update_bot_manager_and_translator(self, bot_manager):
        ...
        try:
            ...
            if (hasattr(bot_manager, 'voice_translator') and 
                ...
                bot_manager.voice_translator.user_processing_enabled = self.user_processing_enabled.copy()
            ...
            logger.debug("Updated bot manager and translator with user processing states")
            return True

        except Exception as e:
            logger.error("Error updating bot manager and translator: %s", str(e))
            return False
